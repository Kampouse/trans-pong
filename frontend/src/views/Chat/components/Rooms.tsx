    ...
    <div className="mt-2 flex w-full">
      <div className="flex flex-col justify-center w-[75px]">
        <div className="w-[40px] mx-auto">
          <Avatar sx={{ weight: 40, height: 40 }}>
            <Person />
          </Avatar>
        </div>
        <div>
          <p className="w-[75px] text-center text-sm text-gray-800">
            {currentMsg.user.username}
          </p>
        </div>
      </div>
      <div className="p-2">
      ...
      {roomDetails.messages.map((currentMsg: Messages) => {
        return (
          userDetails.blockedUsers.find(
            (user) => user.username === currentMsg.user.username
          ) === undefined && <TextBox currentMsg={currentMsg} />
        )
      })}
    </>
  )
