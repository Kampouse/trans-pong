...
import  Typography from '@mui/material/Typography' // change to @mui/material/typegraphy later 
    ...
    React.useEffect(() => {  
      if (send) {
        ...
        if (message !== '' && channelType === ChannelType.privateMessage) {
          ...
          if (msgs) {
            socket.emit('newPrivateMessage', {userId: msgs.user.userID, message: message})
          }
        }
        setSend(false);
        setMessage('');
